...
engine.on("ws", (ev: WsEvent) => broadcast(ev));
...
app.patch("/api/projects/:pid/tasks/:tid/workflow", async (req, res) => {
  ...
  try {
    const task = await taskManager.saveWorkflow(
      project.id,
      project.targetDir,
      req.params.tid,
      req.body
...
const activeExecutions = new Map<string, WorkflowEngine>();
...
app.post("/api/terminal", async (req, res) => {
  ...
  workflowEngine.on("ws", (ev: WsEvent) => broadcast(ev));
